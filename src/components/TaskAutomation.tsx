...
export default function TaskAutomation() {
  ...
  const handleToggleRule = (ruleId: number, currentStatus: string) => {
    ...
    alert(`Automation rule ${ruleId} ${newStatus.toLowerCase()}!`)
  }

  const handleEditRule = (ruleId: number) => {
    console.log('Editing rule:', ruleId)
    alert(`Opening editor for rule ${ruleId}`)
  }

  const handleUseTemplate = (templateId: number) => {
    console.log('Using template:', templateId)
    alert(`Template ${templateId} copied to clipboard!`)
    // In a real app, this would copy the template content
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => {
          ...
          alert('Opening automation rule builder...')
        }}>
                ...
                <TableBody>
                  {automationRules.map((rule) => (
                    <TableRow key={rule.id}>
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          <Button size="sm" variant="outline" onClick={() => handleToggleRule(rule.id, rule.status)}>
                  ...
                  <Card key={template.id} className="border">
                    ...
                    <CardContent>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex space-x-2">
                          <Button size="sm" variant="outline" onClick={() => handleUseTemplate(template.id)}>Use</Button>
                          <Button size="sm" variant="outline" onClick={() => handleEditRule(template.id)}>
                            <Edit className="w-3 h-3" />
                          </Button>
                        </div>
                      </div>
